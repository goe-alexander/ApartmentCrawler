...
public class EmailService implements EmailServiceInterface {
    ...
    public void sendComplexMessageWithInactiveAccounts(List<String> recipients, String subject, String text, List<Apartment> inactiveApartments) throws MessagingException {
        if(inactiveApartments.size() == 0) {
            return;
        }
        for (String recipient : recipients) {
            MimeMessage message = createMimeMessage(subject, inactiveApartments, recipient, mailContentBuilder.buildInactiveApartmentsTables(inactiveApartments));
